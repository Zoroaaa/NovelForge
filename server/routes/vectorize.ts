...
router.post('/clear-all', async (c) => {
  ...
  let totalDeletedFromBinding = 0
  let totalDeletedFromREST = 0
  let errors: string[] = []

  console.log('[clear-all] 开始清空全部向量索引...')

  try {
    if (c.env.VECTORIZE) {
      console.log('[clear-all] 使用 VECTORIZE binding 删除向量...')

      const allLocalVectors = await db.select({ id: vectorIndex.id }).from(vectorIndex).all()
      console.log(`[clear-all] 从 D1 查询到 ${allLocalVectors.length} 条本地记录`)

      if (allLocalVectors.length > 0) {
        const BATCH_SIZE = 100
        for (let i = 0; i < allLocalVectors.length; i += BATCH_SIZE) {
          const batch = allLocalVectors.slice(i, i + BATCH_SIZE)
          const ids = batch.map(v => v.id)

          try {
            await c.env.VECTORIZE.deleteByIds(ids)
            totalDeletedFromBinding += ids.length
            console.log(`[clear-all] 批次 ${Math.floor(i / BATCH_SIZE) + 1}: 成功删除 ${ids.length} 条`)
          } catch (batchError) {
            const errorMsg = `批次 ${Math.floor(i / BATCH_SIZE) + 1} 删除失败: ${(batchError as Error).message}`
            console.error(`[clear-all] ${errorMsg}`)
            errors.push(errorMsg)

            if (ids.length === 1) {
              try {
                await c.env.VECTORIZE.deleteByIds([ids[0]])
                totalDeletedFromBinding += 1
              } catch (singleError) {
                console.error(`[clear-all] 单条删除也失败: ${(singleError as Error).message}`)
              }
            }
          }
        }
      ...
      console.warn('[clear-all] VECTORIZE binding 不可用')
      errors.push('VECTORIZE binding 未配置')
    }

    if (c.env.CLOUDFLARE_API_TOKEN && c.env.CLOUDFLARE_ACCOUNT_ID) {
      console.log('[clear-all] 尝试通过 REST API 清理孤立向量...')
      ...
      let restApiSuccess = false

      do {
        const listUrl = cursor ? `${baseUrl}/vectors?count=1000&cursor=${encodeURIComponent(cursor)}` : `${baseUrl}/vectors?count=1000`
        ...
        try {
          ...
          if (!response.ok) {
            ...
            console.error(`[clear-all] REST API list 失败 (${response.status}): ${errorText}`)
            errors.push(`REST API list 请求失败: HTTP ${response.status}`)
            break
          }

          const data = await response.json() as any
          const vectors: Array<{ id: string }> = data.result?.vectors ?? data.vectors ?? []
          ...
          const ids = vectors.map((v: { id: string }) => v.id)
          console.log(`[clear-all] REST API 获取到 ${ids.length} 条向量，准备删除...`)

          const deleteRes = await fetch(`${baseUrl}/vectors/delete`, {
            method: 'POST',
          ...
          if (deleteRes.ok) {
            totalDeletedFromREST += ids.length
            console.log(`[clear-all] REST API 成功删除 ${ids.length} 条`)
          } else {
            const deleteErrorText = await deleteRes.text()
            console.error(`[clear-all] REST API delete 失败 (${deleteRes.status}): ${deleteErrorText}`)
            errors.push(`REST API delete 请求失败: HTTP ${deleteRes.status}`)
          }

          cursor = data.result?.cursor ?? data.cursor
          if (!cursor || vectors.length < 1000) {
            restApiSuccess = true
            break
          }
        } catch (fetchError) {
          console.error(`[clear-all] REST API 请求异常:`, fetchError)
          errors.push(`REST API 请求异常: ${(fetchError as Error).message}`)
          break
        }
      } while (cursor)

      if (restApiSuccess) {
        console.log('[clear-all] REST API 清理完成')
      }
    } else {
      console.log('[clear-all] Cloudflare API 凭据未配置，跳过 REST API 清理')
    }

    const clearedLocalCount = await db.delete(vectorIndex).run()
    console.log(`[clear-all] 已清空 D1 本地记录: ${clearedLocalCount.meta.changes || '未知'} 条`)

    const totalDeleted = totalDeletedFromBinding + totalDeletedFromREST
    const duration = Date.now() - startTime

    console.log(`[clear-all] ✅ 清空完成! 总计删除: ${totalDeleted} 条 (binding: ${totalDeletedFromBinding}, REST: ${totalDeletedFromREST}), 耗时: ${duration}ms`)

    if (errors.length > 0) {
      console.warn(`[clear-all] ⚠️ 存在 ${errors.length} 个错误:`, errors)
    }

    return c.json({
      ok: true,
      deleted: totalDeleted,
      deletedFromBinding: totalDeletedFromBinding,
      deletedFromREST: totalDeletedFromREST,
      localRecordsCleared: typeof clearedLocalCount === 'number' ? clearedLocalCount : (clearedLocalCount as any)?.meta?.changes || 0,
      durationMs: duration,
      errors: errors.length > 0 ? errors : undefined,
      message: `已清空 ${totalDeleted} 条向量记录（Binding: ${totalDeletedFromBinding}, REST: ${totalDeletedFromREST}），本地记录已清除`
    })
  } catch (error) {
    console.error('[clear-all] ❌ 清空过程发生严重错误:', error)
    return c.json({
      ok: false,
      error: '清空索引失败',
      details: (error as Error).message,
      stack: (error as Error).stack,
      deleted: totalDeletedFromBinding + totalDeletedFromREST
    }, 500)
  }
})
