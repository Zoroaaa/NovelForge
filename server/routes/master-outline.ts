...
/**
 * GET /master-outline/:novelId/history
 * 获取所有历史版本
 */
...
router.post('/', zValidator('json', CreateMasterOutlineSchema), async (c) => {
  ...
  try {
    ...
    const newOutline = await db.insert(masterOutline).values({
    ...
    return c.json({ error: '创建总纲失败', details: (error as Error).message }, 500)
  }
})

/**
 * PUT /master-outline/:id
 * 更新总纲内容（不增加版本号，用于小修改）
 */
router.put('/:id', zValidator('json', z.object({
