...
J. 已有记录约束：资料包"关键词匹配的已知实体"区块中列出的所有实体，其名称、类型、核心属性必须严格沿用，不得重新定义；如该实体附有"⚡ 当前状态"标注，必须以该状态为准进行描写，不得使用过时的初始状态
K. 信息边界约束：资料包"主角状态"区块中"主角已知的关键信息"列出的每一条，主角在本章的行为和决策必须体现已知这些信息；标注"秘密·对外保密"的条目，主角不得在对话中主动透露，但其内心决策必须受到影响`
