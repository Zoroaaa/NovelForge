# NovelForge 开发计划 v3.0

> 基于代码全量审查 · 更新日期：2026-04-30  
...
**设计原则**：四大支柱已夯实，v3.0 专注补完产品化缺口（用户隔离、风格克隆、模板）+ 三大新支柱（一致性引擎/多模态脚本/协作发布）。
...
| Phase D | ⭐ 第五支柱 | 长篇一致性管理（角色状态机） | 百章级长篇的根本性问题 |
...
## Phase D · ⭐ 第五支柱：长篇一致性管理引擎

> **为什么这是第五支柱？**
>
> 现有四大支柱解决了"怎么生成"的问题，但100章以上的长篇面临根本性挑战：角色状态随剧情演变、世界规则因情节需要微调、势力版图随战争改变——这些动态变化无法靠静态角色卡和设定表维护。
>
> 第五支柱的核心是**动态知识库**：随着章节推进，实体状态自动更新，生成下一章时获得的不是初始设定，而是**当前最新状态**。

### D.1 角色状态机

**问题**：现有 `character` 表是静态卡，`powerLevel` 字段虽有境界突破检测，但角色的心态变化、阵营转变、关键经历无法追踪。第50章的角色与第1章相比可能判若两人，但生成时拿到的仍是初始卡。

**数据库变更**：
- 新增 `character_states` 表（characterId, chapterId, stateDelta JSON, summary, createdAt）
- `stateDelta` 记录与上一状态的差异（新增经历/立场变化/心理转变/获得/失去）
- `characters` 表新增 `currentStateVersion`（指向最新 state 快照的 chapterId）

**自动更新机制**：
- postProcess 新增 `step7CharacterStateUpdate`：章节生成后，analysis 模型扫描内容，判断角色状态是否有重大变化
- 变化阈值：立场转变/重大经历/境界突破/人际关系重大变化 → 触发状态快照
- contextBuilder Slot-3（主角状态卡）升级：合并最新 `character_states` 快照

**前端展示**：
- 角色详情页新增"状态时间线"Tab：按章节顺序展示状态演变
- AI监控中心新增"角色状态健康度"：标记长期无更新的主要角色

### D.2 世界动态状态追踪

**问题**：`novelSettings` 是静态设定表，无法记录"势力消亡/地区易主/功法禁用"等剧情驱动的世界变化。

- `novel_settings` 表新增 `stateLog TEXT`（JSON数组，记录每次状态变化 {chapterId, change, reason}）
- postProcess 新增世界状态扫描：检测章节中是否发生设定级别的变更
- contextBuilder Slot-6 设定槽：优先返回含最新 `stateLog` 的设定版本

### D.3 剧情一致性全局检查器

**问题**：现有检查（coherence/consistency/volumeProgress）是章节级的，缺乏跨卷级别的全局一致性检查。

- 新增 `POST /api/novels/:id/global-check` — 全局一致性检查（按需触发，非自动）
- 检查维度：角色跨卷立场一致性、伏笔超期未回收（open状态超过N章）、世界规则前后矛盾
- 新增"一致性报告"页面，问题按严重程度分级，可跳转到对应章节
...
| **10** | **Phase D.1：角色状态机（DB + postProcess step7 + 前端时间线）** | 3天 | Phase D |
| 11 | Phase D.2：世界动态状态追踪 | 2天 | Phase D |
| 12 | Phase D.3：全局一致性检查器 | 2天 | Phase D |
| **13** | **Phase E.1：分镜脚本生成（scriptGen stage + /api/scripts）** | 2天 | Phase E |
| 14 | Phase E.2：有声剧本 + TTS 多角色联动 | 1.5天 | Phase E |
| 15 | Phase E.3：影视剧本（screenwriting格式） | 1天 | Phase E |
| 16 | Phase F.1：协作写作基础（权限模型 + 评论） | 3天 | Phase F |
| 17 | Phase F.2：版本控制 Diff 视图 | 2天 | Phase F |
| 18 | Phase F.3：发布中心 | 1.5天 | Phase F |

---

*NovelForge v3.0 开发计划 · 基于代码全量审查 · 2026-04-30*
