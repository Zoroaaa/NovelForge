...
- 创作工坊深度优化：支持跨阶段数据加载，commit时自动触发向量索引
- 上下文构建器优化：RAG查询文本缩减80%以上
- 摘要生成质量提升：采用结构化格式
...
- **Agent工具系统v2**：从4个工具扩展到5个智能工具（searchChapterHistory、queryCharacterByName、queryForeshadowing、querySettingByName、searchSemantic）
- **创作工坊深度优化**：支持跨阶段数据加载，commit时自动触发向量索引
- **上下文构建器优化**：RAG查询文本聚焦化（控制在800字以内）
- **摘要生成结构化**：章节/大纲/卷摘要采用结构化格式，保留更多关键信息
- **模型参数修复**：temperature 0.72，max_tokens 10000，解决截断和一致性问题
