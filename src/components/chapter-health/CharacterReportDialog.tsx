/**
 * @file CharacterReportDialog.tsx
 * @description 角色一致性检查报告对话框组件
 */
import {
  AlertDialog,
  AlertDialogCancel,
...
import type { CharacterCheckResult, RepairState } from './types'
...
interface CharacterReportDialogProps {
  open: boolean
                ...
                <div className="space-y-5">
                  <div className="flex items-center justify-between p-4 bg-gradient-to-r from-muted/80 to-muted/40 rounded-xl border">
                    <div className="flex items-baseline gap-4">
                      ...
                      <span className={`text-3xl font-bold tabular-nums ${getScoreColor(report.score ?? 100)}`}>
                        {report.score ?? 100}
                      </span>
                      <span className="text-lg text-muted-foreground">/100</span>
                      <span className="text-sm text-muted-foreground ml-2">冲突 {report.conflicts?.length || 0} 个</span>
                    </div>
                  ...
                  <div className="pr-2">
                    <div className="space-y-5 pb-2">
                      {(report.conflicts?.length > 0 || (report.warnings?.filter((w: string) => !w.includes('失败'))?.length ?? 0) > 0) ? (
                        <>
                          {report.conflicts?.map((conflict: any, i: number) => (
                          ...
                          {report.warnings?.filter((w: string) => !w.includes('失败')).map((warning: string, i: number) => (
